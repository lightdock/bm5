...
for i in $(seq 0 $swarms)
  do
    echo "cd swarm_${i}; lgd_generate_conformations.py ../${COMPLEX}_l_u.pdb ../${COMPLEX}_r_u.pdb  gso_100.out 200 --setup ../setup.json > /dev/null 2> /dev/null;" >> generate_lightdock.list;
  done
